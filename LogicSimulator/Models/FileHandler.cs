...
namespace LogicSimulator.Models { 
    public class FileHandler { 
        ...
        public FileHandler() { 
            string app_data = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData); // Получаем путь к папке "AppData"
        ...
        private void AddProject(Project proj) { 
            ...
            if (proj_dict.ContainsKey(path)) return; // Если в словаре уже есть проект с таким путем
        ...
        internal static void SaveProject(Project proj) { 
            ...
            var path = Path.Combine(dir, name); //создаем путь для сохранения
            File.WriteAllText(path, data); //записывает текст из data по нашему пути
        }
        ...
        internal void AppendProject(Project proj) { 
            ...
            var path = Path.Combine(proj.FileDir, proj.FileName); 
            if (project_paths.Contains(path)) return; // проверяет не содержится ли он уже в списке путей проектов project_paths. Если уже содержится, то метод завершается
        ...
        internal Project? SelectProjectFile(Window parent) { 
            ...
            var task = dlg.ShowAsync(parent); // выполняет асинхронную операцию диалога
            var res = task.GetAwaiter().GetResult(); // Результатом является объект Project или null, если пользователь отменил операцию
            if (res == null) return null;

            var path = res[0];
            if (!proj_dict.TryGetValue(path, out var proj)) { // Если проект по указанному пути еще не загружен
